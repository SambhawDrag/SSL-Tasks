...
start_time = None
DIST_THRESH = 75
dboxp = Vector2D(abs(OUR_DBOX_X),0)
...
#LAMBDA FUNCTIONS:-
	...
	def at_DBox(self):
		...
		return not(self.goto_dribble) and target.dist(self.get_pos_asvec2d(curPos)) <= DISTANCE_THRESH*0.1
...
# STATE FUNCTIONS:-
	...
	def on_enter_movetoball(self):
		...
		self.power = 0 
		self.go_at = getPointBehindTheBall(self.kub.state.ballPos, self.theta, -1.5)
	...
	def execute_movetoball(self):
		...
		generatingfunction = _GoToPoint_.execute(start_time, BOT_RADIUS*1.5, True)
		...
		for gf in generatingfunction:
			...
			# # 	break
			if not vicinity_points(self.go_at, target_point ,thresh=BOT_RADIUS*0.5):
				print "Finally!"
				# self.behavior_failed = True 
				break
